...
/* ──────────────────────────────────────────────────────────────
   ...
   follows the reader down the page. The stitch warms from dim ("stored")
   at the spool to full ember ("working") at the needle, and a soft, blurred
   halo leads the drawing tip so the ember bleeds into the faint ghost track
   ahead of it rather than stopping at a hard edge. The needle slowly aims
   toward the incoming thread as the stitch is sewn. A small "Scroll" eyebrow
   fades once they engage. The spool and needle are otherwise eye candy —
   wardrobe depictions that idle with a gentle bob. The whole band stays an
   anchor to the first section, keeping its tap-to-scroll job. Under reduced
   motion the stitch is drawn in full and held, the needle rests aimed, and
   the bob is dropped (MotionConfig reducedMotion="user").
   ────────────────────────────────────────────────────────────── */

// One continuous stitch: gentle, organic undulations that build in amplitude
// across the page — thread under tension, alive the whole way — resolving into
// a tight teardrop that cinches into the needle's eye (the threading moment).
// Authored on a wide, short viewBox and rendered with preserveAspectRatio="none"
// + non-scaling stroke, so it fills the page edge-to-edge at a fixed height while
// the stroke weight and round caps stay crisp.
const THREAD_PATH =
  "M24 52 C 74 51 100 49 150 49 C 205 49 240 55 304 55 C 386 55 426 47 516 47 C 612 47 650 61 742 60 C 824 59 860 39 950 39 C 1048 39 1090 67 1190 66 C 1276 65 1338 53 1414 53 C 1462 52 1484 45 1486 35 C 1488 24 1466 18 1456 26 C 1448 33 1458 46 1490 48 C 1524 50 1558 51 1582 52"
...
export function HeroScrollStroke({
  ...
  // A blurred halo drawn slightly ahead of the sharp stitch, so the ember
  // bleeds into the ghost at the tip instead of capping at a hard edge.
  const haloLength = useTransform(pathLength, (v) => Math.min(1, v + 0.05))
  const labelOpacity = useTransform(scrollYProgress, [0, 0.12], [1, 0])
  // The needle slowly aims toward the incoming thread as the stitch is sewn.
  const needleAim = useTransform(scrollYProgress, [0.2, 0.68], [0, -13])
  const needleRot = useSpring(needleAim, { stiffness: 90, damping: 26, mass: 0.4 })
  ...
  return (
    ...
    >
      ...
      <div className="mt-3 flex w-full items-center gap-2 px-5 sm:gap-4 sm:px-10">
        ...
        >
          <defs>
            {/* dim ("stored") at the spool → full ember → bright ("working") at the needle */}
            <linearGradient id={gradientId} x1="0" y1="0" x2="1" y2="0">
              <stop offset="0" stopColor="color-mix(in srgb, var(--ember) 75%, #000)" />
              <stop offset="0.16" stopColor="var(--ember)" />
              <stop offset="1" stopColor="color-mix(in srgb, var(--ember) 85%, #fff)" />
            </linearGradient>
          </defs>
          {/* ghost track — the stitch's full route, faint ("stored") */}
          <path
            d={THREAD_PATH}
            ...
            opacity={0.5}
          />
          {/* ember bloom — a soft, blurred halo led slightly ahead of the stitch
              so the drawing tip bleeds into the ghost; doubles as the cord's
              outer glow, giving the thread weight */}
          <motion.path
            d={THREAD_PATH}
            ...
            opacity={0.45}
            style={{
              pathLength: reduce ? 1 : haloLength,
              filter: "blur(3px)",
            }}
          />
        ...
        <Bob active={bob} delay={0.4}>
          {/* aim the needle toward the incoming thread; pivot about the eye */}
          <motion.div style={{ rotate: reduce ? -13 : needleRot, transformOrigin: "28% 68%" }}>
            <Needle />
          </motion.div>
        </Bob>
      </div>

// Headless harness for the Clio mobile auth flow.
//
...
async function main() {
  console.log(`\n${YELLOW}=== Clio mobile auth harness ===${RESET}\n`)
  ...
  // -------------------------------------------------------------------------
  const TEST_EMAIL = process.env.TEST_EMAIL || 'clio-mobile-e2e@example.test'
  ...
  const persisted = localStorage.getItem('clio-auth')
  ok('localStorage "clio-auth" key', persisted === null ? 'cleared' : `STILL PRESENT: ${persisted.slice(0, 80)}...`)
  ...
  localStorage.removeItem('clio-auth')
